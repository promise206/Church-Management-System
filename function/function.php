...
function getCollect(){
	...
	while ($row_collect = mysqli_fetch_array($run_collect)) {
		...
		echo '<h1 style="text-align:left;">'.$collect.'</h1>';
	}
}
...
<img src='cworkers/images/$prist_image' width='150' height='150' />
				
			<a href='cworkers/detail.php?view_prist=$prist_id'><button type='submit' name='details' class='btn btn-success'>details</button>
</a>
...
echo "
		...
		<div class='row'>
				...
				<img src='cworkers/images/$chatchist_image' width='150' height='150' />
				
			<a href='cworkers/detail.php?view_pastor=$chatchist_id'><button type='submit' name='details' class='btn btn-success'>details</button>
</a>
				</div>";
			}
		}
...
echo "
	<b><a href='notice.php?notices_id=$notice_id'>$notice_title<br></a></b>
			";
			}
		}
...
function birthday(){
	...
	while ($row_member = mysqli_fetch_array($run_member)) {
		$dob = $row_member['date'];
		$member_name = $row_member['first_name'];
		$member_lastname = $row_member['last_name'];
		$dm = date_parse_from_format("Y-m-d", $dob);
		$month1 = $dm["month"];
		$day = $dm["day"];
		$today   = Date("Y-m-d");
		...
		if (!empty($dob)) {
			if ($month1==$currentMonth) {
				echo "<b style='color: orange'>$member_name $member_lastname($day), </b>";
			}
